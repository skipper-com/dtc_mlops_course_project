...
# Model
The first model for baseline implemented in 01-model.py file. Baseline set by the following steps:
1. Read dataframe (one file = all data)
2. Process features:
* select only neighborhood, room type, number of availability days
* string type for categorical features
* drop records with price out of 95% percentile
3. Split dataframe on train, validation and test datasets
4. Train model and calculating train MSE
...
5. Log MLFlow for experiments with different model's class
6. Print results
...
6. Then I build a deployment which will run on schedule daily at 23:00 using command ```bash prefect deployment build 03-orchestration.py:airbnb_flow -n airbnb --cron "0 23 * * *"```.
7. Next command will make deployment ready to start ```bash prefect deployment apply airbnb_flow-deployment.yaml```.
8. Let's start queue with command ```bash prefect agent start -q 'default'``` and deployment will start on schedule.
9. To be honest steps 6-8 I considered a bit worthless, since data is fixed, there is no need to re-train model. For the real-life project I should consider prefect packed in docker container placed in ECR and run on schedule using prefect server.
10. Last step export run id stored in model folder with command ```bash export RUN_ID="b0f1202cf1da42439539dbf947081ac3"```

# Model Deployment
Easy way to deploy model is to pack all needed components inside container, make a web service (REST API interface) to predict function inside container and it's done.
...
# Model monitoring
Model monitoring is also very important part of MLOps project. Maybe most important because directly controls quality of prediction and so usefulness of service. Following steps describe how can I setup model monitoring for my project:
...
# Best Practices
I followed these steps to implement best practices:
...
4. Prepare Makefile which combines linting, formatting, import sorting, unit and integration testing and publishing container to ECR in the one pipeline file.
5. Prepare .pre-commit-config.yaml for setting of pre-commit hooks.

All main ideas from course in code files. I want to notice only some points:
- need to exclude some file/directories in pyproject.toml for black, because black couldn't parse Dockerfile for example
- need to profile isort in pyproject.toml to fix difference in formatting import between isort and black
- publish container to ECR accessible for Fargate service
...
# Reproducibility
Follow these steps to reproduce the main results:
1. Git clone repo
...
9. Don't forget to export RUN_ID generated by 03-orchestration.py to OS env variable
10. Probably it needs to be fixed some properties and creds in publish process
